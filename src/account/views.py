...
from account.forms import AddPostForm, RegistrationForm, AccountAuthenticationForm, AccountUpdateForm, StatusUpdateForm
from account.models import Account, Post
from friend_system.utils import get_friend_request_or_false
from friend_system.friend_request_status import FriendRequestStatus
from friend_system.models import FriendList, FriendRequest
...
def edit_status_view(request, *args, **kwargs):
	if not request.user.is_authenticated:
		return redirect("login")
	user_id = kwargs.get("user_id")
	account = Account.objects.get(pk=user_id)
...
def gallery(request, *args, **kwargs):
	...
	user_id = kwargs.get("user_id")
	account = Account.objects.get(pk=user_id)
	related_posts = Post.objects.filter(user=account)
	image_list = []
	for img in related_posts:
		image_list.append(img)
	context = {'images': image_list, 'status': account.status}
...
def add_post(request, *args, **kwargs):
	...
	if request.POST:
		...
		if form.is_valid():
			...
			new_post.save()
			return redirect("account:account_home", user_id=account.pk)
		else:
			form = AddPostForm(request.POST, instance=request.user,
				initial={
		...
		context['form'] = form
	context['DATA_UPLOAD_MAX_MEMORY_SIZE'] = settings.DATA_UPLOAD_MAX_MEMORY_SIZE
	return render(request, 'account/add_post.html', context)
